...
function checkWin(index: number) {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    if (
      celdas.value[i * 3] === type &&
      celdas.value[i * 3 + 1] === type &&
      celdas.value[i * 3 + 2] === type
    ) {
      b = 1
      break
    }
  ...
  if (b === 0) {
    if (
      (celdas.value[0] === type && celdas.value[4] === type && celdas.value[8] === type) ||
      (celdas.value[2] === type && celdas.value[4] === type && celdas.value[6] === type)
    ) {
      b = 1
    }
  }
    ...
    <div class="BG">
      <div class="tablero">
        <div
          ...
          :key="index"
          :class="{ 'player-x': celdas[index] === 'X', 'player-o': celdas[index] === 'O' }"
