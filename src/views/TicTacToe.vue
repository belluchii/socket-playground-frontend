...
function checkWin(index: number) {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    if (
      board.value[i * 3] === type &&
      board.value[i * 3 + 1] === type &&
      board.value[i * 3 + 2] === type
    ) {
      b = 1
      break
    }
  ...
  if (b === 0) {
    if (
      (board.value[0] === type && board.value[4] === type && board.value[8] === type) ||
      (board.value[2] === type && board.value[4] === type && board.value[6] === type)
    ) {
      b = 1
    }
  }
...
<template>
  <div id="Main">
    ...
    <p :class="['waitMessage', waitMessage ? '' : 'd-none']">
    ...
    <div class="BG">
      <div class="tablero">
        <div
          class="celda"
          v-for="(celda, index) in 9"
          :key="index"
          :class="{ 'player-x': board[index] === 'X', 'player-o': board[index] === 'O' }"
          @click="MarkPlace(index)"
        ></div>
      </div>
    </div>
  </div>
...
.celda.player-o {
  ...
  background-image:
    ...
    radial-gradient(circle, var(--color-white) 32%, transparent 33%);
